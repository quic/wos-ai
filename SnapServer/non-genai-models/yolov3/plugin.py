...
"""
YOLOv3 plugin — object detection.
...
class YoloV3Plugin(VenvPlugin):
    ...
    def image_variation(self, image: bytes, n: int = 1, size: str = "1024x1024",
        ...
        if response_format == "url":
            from pipeline_core import draw_detection_boxes
            png = draw_detection_boxes(
                self._pre._last_rgb_uint8, result.boxes, result.scores, result.labels
            )
            return [{"image_bytes": png}]

        detections = [
            {"label": lbl, "score": float(s), "box": b.tolist()}
            for lbl, s, b in zip(result.labels, result.scores, result.boxes)
        ]
        return [{"b64_json": json.dumps({"detections": detections})}]
